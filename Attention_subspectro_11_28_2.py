...
#加载数据集：训练集及测试集
data_train = torchvision.datasets.ImageFolder('./img_train/', transform = transforms.Compose([transforms.Scale((img_size, img_size)), transforms.ToTensor()]))
data_test = torchvision.datasets.ImageFolder('./img_test/', transform = transforms.Compose([transforms.Scale((img_size, img_size)), transforms.ToTensor()]))
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.base_conv = nn.Sequential(
            ...
            nn.Conv2d(4, 8, kernel_size = 5, stride = 1, padding = 2, bias = False),
            nn.BatchNorm2d(8),
            nn.ReLU(),
            # nn.MaxPool2d(stride = 2, kernel_size = 2)
            nn.AvgPool2d(2, 2),
...
for epoch in range(n_epochs):
    ...
    for data in data_loader_train:
        ...
        i += 1
        if i % 10 == 0:
            print('i:{} loss: {} correct: {}'.format(i, loss.data, batch_correct / batch_size))
